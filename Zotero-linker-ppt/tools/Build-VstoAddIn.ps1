...
Import-Module (Join-Path $windowsPowerShellModules "Microsoft.PowerShell.Security\Microsoft.PowerShell.Security.psd1") -ErrorAction Stop
...
$projectPath = Join-Path $projectRoot "Zotero-linker-ppt.csproj"
$certificateSubject = "CN=Zotero Linker VSTO"
...
function Find-VstoBuildEnvironment {
    ...
    if ($RequestedMSBuildPath) {
        ...
        foreach ($candidate in @(
            ...
            "D:\Working Program\VS2026\Microsoft Visual Studio\18\Community"
        )) {
...
if (-not (Test-Path -LiteralPath $projectPath)) {
    throw "VSTO project was not found: $projectPath"
}
...
$certificatePath = if ($CertificateOutputPath) {
    ...
    Join-Path $projectRoot "installer\vsto-signing.cer"
}
...
Set-ManifestCertificate -ProjectPath $projectPath -Thumbprint $certificate.Thumbprint

$versionParts = $Version.Split(".")
while ($versionParts.Count -lt 4) {
    $versionParts += "0"
}
$applicationVersion = ($versionParts[0..3] -join ".")
...
& $environment.MSBuildPath $projectPath `
    "/restore" `
...
foreach ($required in @("Zotero-linker-ppt.vsto", "Zotero-linker-ppt.dll", "Zotero-linker-ppt.dll.manifest")) {
    $requiredPath = Join-Path $outputDirectory $required
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Build output is missing: $requiredPath"
    }
}
