...
Zotero Linker 是一个面向 Windows 的 Word 文献引用导航插件，用于增强 Zotero 在长文档写作、阅读和审阅中的使用体验。插件已针对 Microsoft Office Word 和 WPS Word/Writer 的使用场景进行适配，适合论文、综述、学位论文、基金申请书和技术报告等包含大量参考文献的文档。

## 解决的 Gap

Zotero 可以很好地完成文献管理、正文引用插入和参考文献列表生成，但在 Word 文档中仍然存在一个常见痛点：正文中的引用编号和文末参考文献之间缺少便捷的导航关系。

在长文档中，用户经常需要从正文中的 `[1]`、`[2-4]` 或 `[3, 5, 8]` 跳转到对应参考文献，或者从参考文献列表返回正文中引用它的位置。传统方式通常需要手动滚动和查找，文献数量越多，阅读、修改和审稿效率越低。
...
- 支持压缩引用处理：对于 `[2-4]` 这类连续引用，插件会结合 Zotero 域信息处理可见和隐藏的引用项。
...
- 状态反馈：在 Word 功能区显示链接数量、反向链接数量、失败项和格式修复结果。
...
本插件主要面向 Windows 桌面端写作场景，适用于使用 Zotero 管理参考文献并在 Word/WPS 中撰写长文档的用户。
